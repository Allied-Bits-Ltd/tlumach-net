...
- [NEW] The icons of the Visual Studio extension follow the monochrome style of Visual Studio 2026 instead of the previous teal and orange. The command icons are drawn in a neutral near-black; the Visual Studio image service lightens them on a dark background, which the image manifest permits. The item template icon is a mid tone, because the Add New Item dialog draws the icon of a template as a plain image file and never re-themes it. The colorful Tlumach logo is unchanged - it identifies the extension in the marketplace and in the Extensions manager, where a product logo is expected to be in color.
