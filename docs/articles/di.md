...
An alternative approach is to use [Generator](generator.md) to generate <xref:Tlumach.TranslationUnit> instances and then use those instances as keys. As you will see below, Tlumach supports this model that provides grounds for syntax checking and avoiding typos. 
...
The second localizer uses options provided for the given scope. The "Strings" type here is an autogenerated class that includes a property named "TranslationManager". The IStringLocalizer implementation will take the <xref:Tlumach.TranslationManager> instance and use it to find the translated string with the "Welcome" name. The ues of the autogenerated class also enables referencing translation units for the purpose of syntax checking.

## More About Options

<xhref:Tlumach.Extensions.Localization.TlumachLocalizationOptions> objects include different properties that let you reference an instance of <xref:Tlumach.TranslationManager> or create a new instance. If the <xhref:Tlumach.Extensions.Localization.TlumachLocalizationOptions.TranslationManager> property is set, an IStringLocalizer implementation will use the value from this property and ignore the other settings. If the <xhref:Tlumach.Extensions.Localization.TlumachLocalizationOptions.TranslationManager> property is null and <xhref:Tlumach.Extensions.Localization.TlumachLocalizationOptions.TranslationConfiguration> is not null, an IStringLocalizer implementation will use this configuration to create a new instance of <xref:Tlumach.TranslationManager>. And if <xhref:Tlumach.Extensions.Localization.TlumachLocalizationOptions.TranslationConfiguration> is null, the other properties are used to create an instance of <xref:Tlumach.TranslationConfiguration> and then this new configuration is used to create an instance of <xref:Tlumach.TranslationManager>. Note that in the latter scenario you don't need a [configuration file](config-file.md), you need only the default translation file.

Each instance of <xhref:Tlumach.Extensions.Localization.TlumachLocalizationOptions> may include a reference to a different [translation set](glossary.md#TranslationSet). 
