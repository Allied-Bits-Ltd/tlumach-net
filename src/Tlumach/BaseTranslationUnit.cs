...
public class BaseTranslationUnit
{
    ...
    /// <summary>
    /// Returns the text of the template translation entry without processing the template. This may be useful when template processing is handled by the caller (e.g., when strings use .NET template format which is handled using the <see cref="string.Format(string, object[])"/> method or its overloads).
    /// </summary>
    ...
    /// <summary>
    /// Gets the unprocessed template of the unit for the current culture set in the associated translation manager.
    /// <para>This is the counterpart of <see cref="CurrentValue"/> for a caller that formats the text itself, which is what the string accessors generated by
    /// <c>createStringAccessors</c> need: an attribute that localizes through a resource type hands the text to <see cref="string.Format(string, object[])"/> together with its own arguments.</para>
    /// </summary>
    public string CurrentTemplate => GetValueAsTemplate(TranslationManager.CurrentCulture);
